...
/// `#include <stdio.h>` → local_name="stdio", path="stdio.h"
...
mod tests {
    ...
    fn parse_imports_code(code: &str, map: &mut ModuleMap) {
        let tree = parser::parse(code, Language::C).unwrap();
    ...
    fn test_system_include() {
        ...
        parse_imports_code("#include <stdio.h>\n", &mut map);
        assert_eq!(map.imports.get("stdio"), Some(&"stdio.h".to_string()));
    }
